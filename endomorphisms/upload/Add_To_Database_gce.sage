# Main upload file for gce format

#inputfile = 'gce_lmfdb_output.txt'
# For testing and debugging:
inputfile = 'gce_delta_output.txt'
...
while counter_done < num_lines:
    ...
    g2db = lmfdb.genus2_curves.curves
        ...
        try:
            ...
            for line in inputstream:
                ...
                if counter_current > counter_done:
                    ...
                    min_eqn = [ R(p).list() for p in eval(linesplit[3]) ]
                    curve = g2db.find_one({'min_eqn': eval(str(min_eqn))})
                    endodata = {'label': curve['label']}

                    # Input endomorphism lattice and prettify:
                    EDs = eval(linesplit[9])
                    EDs_se = sage_eval(linesplit[9])
                    N = len(EDs)
                    for i in range(N):
