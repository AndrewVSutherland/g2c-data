...
function GeometricEndomorphismBasisFromPeriods(P : epscomp := epscomp0,
...
function GeometricEndomorphismBasis(h, g : prec := prec0, epscomp := epscomp0,
    epsLLL := epsLLL0, epsinv := epsinv0);
// Input:   Two polynomials h and g over the rationals, reals or CC.
// Output:  An approximate basis of the endomorphism ring of the hyperelliptic
//          curve with equation y^2 + h*y = g, returned in both analytic and
//          rational representations, and the period matrix.

R := PolynomialRing(ComplexField(prec));
f := (R!g) + ((R!h)/2)^2;
J := AnalyticJacobian(f);
P := Transpose(BigPeriodMatrix(J));
As, Rs := GeometricEndomorphismBasisFromPeriods(P : epscomp := epscomp,
    epsLLL := epsLLL, epsinv := epsinv);
