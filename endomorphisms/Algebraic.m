...
function EndomorphismLatticeG2(AsAlg, As, Rs : Geometric := true, AddTensor :=
    false, AddRing := false, AddSatoTate := false, AddSplitting := false);
...
Gp, Gf, Gphi := AutomorphismGroupFixed(L);
...
for tup in Ktups do
    K := tup[1];
    if K ne L then
        // FIXME: Replace bugged Magma version
        //Hp := FixedGroup(L, K);
        ...
        EndRep, idemsList := EndomorphismData(GeoEndList, L, K, Gp, GensHp,
            ...
            AddRing, AddSatoTate := AddSatoTate, AddSplitting := AddSplitting);
        // Keep track of field of definition of splitting so as to return it
...
// AddTensor, AddRing and AddSplitting, information is returned about the
// endomorphism ring (an order in the endomorphism algebra), the endomorphism
// algebra tensored with RR over QQ, and the idempotents needed for a splitting
...
if g eq 2 then
    ...
    if AddSplitting then
        idems := SplittingIdempotentsG2(idemsC, GensC, FactorsQQ, CToPretty,
            EndList0);
    else
        idems := [* [], [], [] *];
    end if;
    ...
    Append(~ED, EndomorphismAlgebraFactorsGeneric(C));
    if AddRing then
        // If asked for information about the order, we merely return its
        // discriminant:
        Append(~ED, EndomorphismRingGeneric(GensC));
    end if;
    // In general no quick option for AddTensor or AddSplitting suggests itself,
...
// later use in determining the endomorphism ring and splittings.
...
function SplittingIdempotentsG2(idemsC, GensC, FactorsQQ, CToPretty,
    EndList0);
// Returns idempotents from which a splitting can be reconstructed.
...
// First possibility: central splitting of the algebra gives unique idempotents.
